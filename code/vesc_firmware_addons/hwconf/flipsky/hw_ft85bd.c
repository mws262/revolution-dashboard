...
void hw_init_gpio(void) {
	...
	palSetPadMode(BRAKE_LIGHT_GPIO, BRAKE_LIGHT_PIN,
					PAL_MODE_OUTPUT_PUSHPULL);
	palSetPadMode(FRONT_LIGHT_GPIO, FRONT_LIGHT_PIN,
					PAL_MODE_OUTPUT_PUSHPULL);
	palSetPadMode(HORN_GPIO, HORN_PIN,
					PAL_MODE_OUTPUT_PUSHPULL);

	palClearPad(BRAKE_LIGHT_GPIO, BRAKE_LIGHT_PIN); // Needs to be explicitly set to low, or the lights come on.
	palClearPad(FRONT_LIGHT_GPIO, FRONT_LIGHT_PIN);
	palClearPad(HORN_GPIO, HORN_PIN);
...
// Largest alterations for the FT85BD:
void hw_setup_adc_channels(void) {
	...
	ADC_RegularChannelConfig(ADC2, ADC_Channel_5, 2, ADC_SampleTime_15Cycles);
	...
	ADC_InjectedChannelConfig(ADC1, ADC_Channel_8, 3, ADC_SampleTime_15Cycles);
	ADC_InjectedChannelConfig(ADC2, ADC_Channel_9, 3, ADC_SampleTime_15Cycles);
...
void hw_try_restore_i2c(void) {
	return; // trying this to see if it's behavior is affecting the light pins.
	// if (i2c_running) {
	// 	i2cAcquireBus(&HW_I2C_DEV);
